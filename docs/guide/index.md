...
`prudent-mcp` is an [MCP](https://modelcontextprotocol.io) server that exposes a structured knowledge base for IRB credit-risk model validation — regulation, statistical tests, supervisor checks, validation playbooks, and a registry of the source documents behind them — to any LLM client that speaks MCP.
...
`prudent-mcp` provides that structured access. It defines five surfaces — regulation, tests, checks, playbooks, sources — each with its own URI scheme, and exposes them through MCP's standard tool/resource/prompt protocol. Any compliant client (Claude Desktop, Claude Code, Cursor, custom integrations) can plug it in.
...
- **Versioning only on regulation.** Tests, checks, and playbooks track `last_updated` for audit but are always served latest. Sources track supersession as a status + pointer — not version history.
...
- **prudent corpus** — the curated body of regulation, tests, checks, playbooks, and sources that adapters serve up.
