...
- **The search envelope** — the four `search_*` tools share `{ results, returned, total_matches, offset, truncated, next_offset, notice }` with `limit` (default 20, max 100) and `offset` paging. `returned` is this page; `total_matches` is the whole match set — ranking is uncapped, so page with `next_offset` until it is null before concluding anything is absent. Queries are ranked and field-scoped, minimum 2 characters — search never enumerates the corpus; that's what `list_*` tools and traversal are for.
